...
def check_password_validity(tran_pwd):
    if (6 <= len(tran_pwd) <= 12) and re.search("[a-z]", tran_pwd) and re.search("[0-9]", tran_pwd) and \
            re.search("[A-Z]", tran_pwd) and re.search("[$#@]", tran_pwd):
        return True
    else:
        return False


pwd_seq = input("Enter the password sequence: ")
valid_pwd = []
for pwd in pwd_seq.split(","):
    if not check_password_validity(pwd):
        continue
